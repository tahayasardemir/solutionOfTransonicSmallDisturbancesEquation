...
c     alpha   = 0.
      ...
      print *, 'Input 1 for Jacobi, 2 for SOR, 3 for SLOR: '
      read(*,*) method
...
c..delta_phi for SOR
         ...
         Res   = Rhs(j)  - dphi(i-1,j)*Cs(j) - dphi(i-2,j)*Cr(j) -
     &   dphi(i,j-1)*Cv(j)
         dphi(i,j) = omega*Res/Ct(j)
c..residual for SLOR
         elseif (method.eq.3) then
        Res3(j) = omega*Rhs(j)  - dphi(i-1,j)*Cs(j) - dphi(i-2,j)*Cr(j)
         endif
...
c..Output Phi,u,v,pressure,density,Mach in TECPLOT format
      write(1,*) ' variables="x","y","phi","mach", "pressure", "u", "v"'
      write(1,*) ' zone i=',ig, ',j=',jg
      do j = 1,jg
      do i = 1,ig
         write(1,'(8E12.4)') xg(i),yg(j),phi(i,j),mach(i,j),
     +     pressure(i,j), uflow(i,j), vflow(i,j)
      enddo
      enddo
...
c..The wake condition is applied in here
      ...
      if ((abs(circ) .gt. 0.0001)) THEN
      do i=1,ig
         phi(i,1)=circ/(2*pi*(1-fsmach2))*atan(sqrt(1-fsmach2)*
     >    (yg(1)-0.0)/(xg(i)-0.25))
         phi(i,jg)=circ/(2*pi*(1-fsmach2))*atan(sqrt(1-fsmach2)*
     >             (yg(jg)-0.0)/(xg(i)-0.25))
      end do

      do j=1,jg
         phi(1,j)=circ/(2*pi*(1-fsmach2))*atan(sqrt(1-fsmach2)*
     >            (yg(j)-0.0)/(xg(1)-0.25))
         phi(ig,j)=circ/(2*pi*(1-fsmach2))*atan(sqrt(1-fsmach2)*
     >             (yg(j)-0.0)/(xg(ig)-0.25))
      end do
      ENDIF
...
c-------------------------------------------------------------------
      ...
      phix = (phi(i+1,j)-phi(i-1,j))/(dx(i)+dx(i+1))
